          ...
          <div className="mb-4 p-3.5 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900/60 rounded-xl text-amber-900 dark:text-amber-200 text-xs space-y-2.5">
            <div className="flex items-start gap-2.5">
              ...
              <div className="space-y-1.5 flex-1">
                ...
                {unauthorizedDomain && (
                  <div className="text-[11px] text-slate-600 dark:text-slate-300 leading-normal space-y-1.5 bg-white/80 dark:bg-slate-900/80 p-2.5 rounded-lg border border-amber-200/80 dark:border-amber-900/40">
                    <div>
                      ⚠️ In Firebase Console, enter <strong>ONLY</strong> this exact domain:
                    </div>
                    <div className="flex items-center justify-between gap-2 font-mono font-bold text-slate-900 dark:text-white bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded text-xs">
                      <span className="select-all">{unauthorizedDomain}</span>
                      <button
                        type="button"
                        onClick={() => {
                          navigator.clipboard.writeText(unauthorizedDomain);
                          alert('Copied: ' + unauthorizedDomain);
                        }}
                        className="text-[10px] bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 px-2 py-0.5 rounded text-slate-700 dark:text-slate-200 cursor-pointer"
                      >
                        Copy
                      </button>
                    </div>
                    <div className="text-[10px] text-rose-600 dark:text-rose-400 font-medium">
                      ❌ Do NOT type "https://" and do NOT type "/erpGUD/". Firebase rejects anything with https:// or slashes as "not a valid domain".
                    </div>
                  </div>
                )}
              </div>
            ...
            >
              ...
              <span>Skip & Enter ERP Directly as Owner</span>
            </Button>
          </div>
        )}

        {/* Primary Action: 1-Click Instant Owner Access & Google */}
        <div className="mb-5 space-y-2.5">
          <Button
            type="button"
            onClick={handleAnonymousSignIn}
            disabled={loading}
            className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold flex items-center justify-center gap-2 shadow-md transition-all cursor-pointer"
          >
            <Sparkles className="w-4 h-4 text-amber-300" />
            <span>🚀 Enter ERP as Owner (Instant 1-Click • No Pass)</span>
          </Button>
          ...
          <Button
            ...
            disabled={loading}
            className="w-full py-2.5 bg-white hover:bg-slate-50 dark:bg-slate-800 dark:hover:bg-slate-750 text-slate-700 dark:text-white border-slate-300 dark:border-slate-700 rounded-xl text-xs font-semibold flex items-center justify-center gap-2 shadow-sm transition-all cursor-pointer"
          >
            ...
            <span>Or Connect with Google Account</span>
          </Button>

          <p className="text-[10px] text-center text-slate-400">
            Immediate entry with complete ERP, live Google Sheets, inventory & financial permissions
          </p>
        </div>
